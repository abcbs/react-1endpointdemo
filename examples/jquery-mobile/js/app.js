...
var PageOneContent = React.createClass({
  ...
  render: function() {
    return React.DOM.div(null,
      ...
      React.DOM.p(null, 'This is a multi-page boilerplate template that you can copy to build your first jQuery Mobile page. ' +
          'This template contains multiple "page" containers inside, unlike a single page template that has just one page within it.'),
      React.DOM.p(null, 'Just view the source and copy the code to get started. ' +
          'All the CSS and JS is linked to the jQuery CDN versions so this is super easy to set up. ' +
          'Remember to include a meta viewport tag in the head to set the zoom level.'),
      React.DOM.p(null,
  ...
  render: function() {
    return React.DOM.div(null,
      ...
      React.DOM.p(null, 'Notice that the theme is different for this page because we\'ve added a few ',
        ...
        ' swatch assigments here to show off how flexible it is. ' +
        'You can add any content or widget to these pages, but we\'re keeping these simple.'),
      JQueryMobileButton({href:'#one', 'data-direction':'reverse',
                            className:'ui-btn ui-shadow ui-corner-all ui-btn-b'}, 'Back to page "one"')
    );
  }
});
...
var PagePopUpContent = React.createClass({
  ...
  render: function() {
    return React.DOM.div(null,
      ...
      JQueryMobileButton({href:'#one', 'data-rel':'back',
                            className:'ui-btn ui-shadow ui-corner-all ui-btn-inline ui-icon-back ui-btn-icon-left'}, 'Back to page "one"')
                            /**上面的class为多类选择器*/
    );
  }
});
